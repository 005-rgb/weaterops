...
function App() {
  ...
  return <div className="site-shell">
    ...
    <main>
      ...
      <section className="workspace-section" id="workspace"><div className="workspace-head"><div><div className="section-kicker">{t.op}</div><h2>{t.start}<br /><span>{t.plan}</span></h2></div><p>{t.startDesc}</p></div><LocationPickerMap onSelect={setPicked} lang={lang} />{picked && <p className="selected">{t.location} <strong>{picked.fullName}</strong> · {picked.code}</p>}<TrackingMap analyses={demo} /></section>
    ...
    <footer><a className="brand" href="#"><span className="brand-mark"><span>W</span></span><span>weather<span className="brand-muted">ops</span></span></a><span>{t.tagline}</span><span>© 2026 WeatherOps</span></footer>
  </div>
}
